...
mkdir -p notebooks_executed/preliminary-experiment/retention/backwards

for start_month in 1; do
    for start_day in 26 31; do #1 6 11 16 21
    # run for single notebook and put into background
        ...
        && papermill --cwd notebooks \
            notebooks/preliminary-experiment/2023-09-17_retention-split-backwards.ipynb \
            notebooks_executed/preliminary-experiment/retention/backwards/2023-09-17_retention-split-2009-${start_month}-${start_day}.ipynb \
...
for start_month in 6; do
    for start_day in 10 20 25; do # 5 15 30
    # run for single notebook and put into background
        srun --ntasks=1 --exclusive singularity run -B /sfs -B /gxfs_work1 -B $PWD:/work --pwd /work parcels-container_2022.07.14-801fbe4.sif bash -c \
        ". /opt/conda/etc/profile.d/conda.sh && conda activate base \
        && papermill --cwd notebooks \
